...
imgPath = os.path.join(yolo_dir, 'img0151.jpg')  # 测试图像
...
if len(idxs) > 0:
    for i in idxs.flatten(): # indxs是二维的，第0维是输出层，所以这里把它展平成1维
        ...
        cv2.rectangle(img, (x, y), (x+w, y+h), color, 5)  # 线条粗细为2px
        text = "{}: {:.4f}".format(labels[classIDs[i]], confidences[i])
        cv2.putText(img, text, (x, y-20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 5)  # cv.FONT_HERSHEY_SIMPLEX字体风格、0.5字体大小、粗细2px
cv2.imwrite("./jim.jpg",img)
#cv2.imshow('目标检测结果', img)
#cv2.waitKey(0)
